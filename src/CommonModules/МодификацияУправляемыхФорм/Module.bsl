...
Процедура ОграничитьИзменениеРеквизита(ЭлементФормы, Имя = Неопределено, СоответствиеВключить, СоответствиеИсключить, Префикс = "", ТаблицаОграничения, Форма)
	Если ЭлементФормы = Неопределено тогда
		Возврат;	
	КонецЕсли;
	
	Если Имя <> Неопределено Тогда
		ИмяЭлемента = ВРЕГ(Имя);
	Иначе
...
Функция ВключитьРеквизит(ИмяТЧ = Неопределено, ИмяРеквизита, СоответствиеВключить, СоответствиеИсключить, ВключениеПоУмолчанию = Истина)
	Если ТипЗнч(ИмяТЧ) = Тип("Массив") Тогда
		МассивПрефиксы = ИмяТЧ;
	Иначе
		Если ЗначениеЗаполнено(ИмяТЧ) Тогда
			МассивПрефиксы = СтрРазделить(ИмяТЧ, ".");
		Иначе
			МассивПрефиксы = Новый Массив;
		КонецЕсли;
	КонецЕсли;
	...
	Для Инд = 1 По МассивПрефиксы.Количество() Цикл
		ПрефиксТЧ = ?(ТипЗнч(ИмяТЧ) = Тип("Массив"), "", ПрефиксТЧ) + МассивПрефиксы[Инд - 1] + ?(ЗначениеЗаполнено(МассивПрефиксы[Инд - 1]), ".", "");
		
		Если СоответствиеВключить[ВРег(ПрефиксТЧ) + "*"] <> Неопределено Тогда
			ИндВключения = Инд;
		КонецЕсли;
		Если СоответствиеИсключить[ВРег(ПрефиксТЧ) + "*"] <> Неопределено Тогда
			ИндИсключения = Инд;
		КонецЕсли;
	КонецЦикла;
	...
	Если СоответствиеВключить[ВРег(ПрефиксТЧ + ИмяРеквизита)] <> Неопределено Тогда
		ИндВключения = МассивПрефиксы.Количество() + 1;
	КонецЕсли;
	Если СоответствиеИсключить[ВРег(ПрефиксТЧ + ИмяРеквизита)] <> Неопределено Тогда
		ИндИсключения = МассивПрефиксы.Количество() + 1;
	КонецЕсли;
...
Процедура ДобавитьОграничение(ТаблицаОграничения, ЭлементФормы, ИмяСвойства, Значение)
	МассивСтрок = ТаблицаОграничения.НайтиСтроки(Новый Структура("ИмяЭлемента,ИмяСвойства", ЭлементФормы.Имя, ИмяСвойства));
