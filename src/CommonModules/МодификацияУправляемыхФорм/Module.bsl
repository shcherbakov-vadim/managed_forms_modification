			...
			Для Каждого ЭлементСтруктуры Из СтруктураОперации Цикл
				...
				ИначеЕсли ЭлементСтруктуры.Ключ = "СписокВыбора" Тогда
					Если Реквизит.ТипЗначения.Типы().Количество() = 1 Тогда
						...
						Если МетаданныеРеквизита <> Неопределено Тогда
							Для Каждого Имя Из СтрРазделить(ЭлементСтруктуры.Значение, ",") Цикл
								Элемент.СписокВыбора.Добавить(ПредопределенноеЗначение(МетаданныеРеквизита.ПолноеИмя() + "." + СокрЛП(Имя)));
							КонецЦикла;
						Иначе
							Для Каждого Описание Из ПолучитьМассивОписаний(ЭлементСтруктуры.Значение) Цикл
								Элемент.СписокВыбора.Добавить(Вычислить(Описание));
							КонецЦикла;
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
...
Процедура ПрименитьШаблон(ПараметрыОбработки, ТипШаблона, ТекстВызова, ИмяРодителя = Неопределено)
	...
	ПараметрыОбработкиШаблона = Новый Структура("Форма,МакетИзменений");
