...
Процедура СкопироватьПодчиненныеЭлементы(Форма, ПараметрыОбработки, ИсходныйЭлемент, Элемент, ПеремещатьГруппы = Истина)
		...
		Для Каждого ПодчиненныйЭлемент Из ИсходныйЭлемент.ПодчиненныеЭлементы Цикл
			Если ТипЗнч(ПодчиненныйЭлемент) = Тип("КнопкаФормы")
				...
				ИЛИ ТипЗнч(ПодчиненныйЭлемент) = Тип("ГруппаФормы")
					И ПеремещатьГруппы Тогда
				
				Форма.Элементы.Переместить(ПодчиненныйЭлемент, Элемент);
			Иначе
				Если СтрНачинаетсяС(ПодчиненныйЭлемент.Имя, ИсходныйЭлемент.Имя)
					И (ТипЗнч(ПодчиненныйЭлемент) <> Тип("ПолеФормы")
						ИЛИ СтрНачинаетсяС(ПодчиненныйЭлемент.ПутьКДанным, ИсходныйЭлемент.ПутьКДанным)) Тогда
						
					ИмяЭлемента = Сред(ПодчиненныйЭлемент.Имя, СтрДлина(ИсходныйЭлемент.Имя) + 1);
					НовыйЭлемент = Форма.Элементы.Добавить(Элемент.Имя + ИмяЭлемента, ТипЗнч(ПодчиненныйЭлемент), Элемент);
					
					Если ТипЗнч(ПодчиненныйЭлемент) = Тип("ПолеФормы") Тогда
						ИмяРеквизита = Сред(ПодчиненныйЭлемент.ПутьКДанным, СтрДлина(ИсходныйЭлемент.ПутьКДанным) + 2);
						НовыйЭлемент.ПутьКДанным = Элемент.ПутьКДанным + "." + ИмяРеквизита;
					КонецЕсли;
					...
					СкопироватьПодчиненныеЭлементы(Форма, ПараметрыОбработки, ПодчиненныйЭлемент, НовыйЭлемент, ПеремещатьГруппы И ТипЗнч(ИсходныйЭлемент) = Тип("ГруппаФормы"));
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция ИсключаемыеСвойстваЭлемента(ПараметрыОбработки, Элемент)
	Если НЕ ПараметрыОбработки.Свойство("ИсключаемыеСвойстваЭлементов") Тогда
		...
		ИсключаемыеСвойства.Вставить(Тип("ПолеФормы"), "Имя,ПутьКДанным,ВыделенныйТекст,ТекстРедактирования");
	...
	Возврат ИсключаемыеСвойства[ТипЗнч(Элемент)];
КонецФункции
