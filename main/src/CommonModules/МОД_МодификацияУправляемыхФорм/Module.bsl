...
Процедура ПрименитьМакетИзменений(Форма) Экспорт
	...
	ПослеПримененияМакетаИзменений(Форма);
КонецПроцедуры

Процедура ПослеПримененияМакетаИзменений(Форма)
КонецПроцедуры
...
Процедура ИзменитьРеквизиты(ПараметрыОбработки, МакетИзменений, ИмяРодителя = Неопределено, ШиринаИмени = Неопределено, НомСтроки = 2, НомКолонки = 1)
	...
	Если ТипЗнч(ПараметрыОбработки.ДанныеМакета) = Тип("Соответствие") Тогда
		Для Каждого КоллекцияИзменений Из МакетИзменений Цикл
			Если НЕ ПрименятьИзменение(Форма, "Реквизиты", КоллекцияИзменений) Тогда
				Продолжить;
			КонецЕсли;
			
			ИмяРеквизита = КоллекцияИзменений["Имя"];
			...
			Иначе
				...
				Для Каждого ЭлементСоответствия Из КоллекцияИзменений Цикл
	...
	Иначе
		...
		Пока Истина Цикл
			КоллекцияИзменений = Новый Соответствие;
			КоллекцияИзменений.Вставить("Имя", ИмяРеквизита);
			Для Инд = ШиринаИмени + 1 По МакетИзменений.ШиринаТаблицы Цикл
				...
				Если ЗначениеЗаполнено(ОписаниеЗначения) Тогда
					КоллекцияИзменений.Вставить(ИмяСвойства, ОписаниеЗначения)
				КонецЕсли;
			КонецЦикла;
			...
			Если НЕ ПрименятьИзменение(Форма, "Реквизиты", КоллекцияИзменений) Тогда
				Пока Истина Цикл
					...
					Если НомСтроки > МакетИзменений.ВысотаТаблицы Тогда
						Возврат;
					ИначеЕсли ЗначениеЗаполнено(МакетИзменений.Область(НомСтроки, НомКолонки).Текст) Тогда
						Прервать;
					Иначе
						Для Инд = 1 По НомКолонки - 1 Цикл
...
Процедура ИзменитьСвойствоРеквизита(Форма, ПараметрыОбработки, РеквизитФормы, ИмяРеквизита, СвойстваРеквизитов, ИмяСвойства, Знач ОписаниеЗначения, СтруктураСвойства, СтруктураОперации)
...
Процедура ИзменитьКоманды(ПараметрыОбработки, МакетИзменений)
	...
	Если ТипЗнч(ПараметрыОбработки.ДанныеМакета) = Тип("Соответствие") Тогда
		Для Каждого КоллекцияИзменений Из МакетИзменений Цикл
			Если НЕ ПрименятьИзменение(Форма, "Команды", КоллекцияИзменений) Тогда
				Продолжить;
			КонецЕсли;	
			
			ИмяКоманды = КоллекцияИзменений["Имя"];
	...
	Иначе
		Для НомСтроки = 2 По МакетИзменений.ВысотаТаблицы Цикл
			...
			КоллекцияИзменений = Новый Соответствие;
			КоллекцияИзменений.Вставить("Имя", ИмяКоманды);
			...
			Иначе
				...
				Для Каждого ЭлементСоответствия Из КоллекцияИзменений Цикл
					ИзменитьСвойствоКоманды(ПараметрыОбработки, Команда, ИмяКоманды
						,ЭлементСоответствия.Ключ, ЭлементСоответствия.Значение, СтруктураСвойства);
				КонецЦикла;
...
Процедура ИзменитьСвойствоКоманды(ПараметрыОбработки, Команда, ИмяКоманды, ИмяСвойства, Знач ОписаниеЗначения, СтруктураСвойства)
	ЗаполнитьПараметры(ПараметрыОбработки, ОписаниеЗначения);
	...
	Если Поз > 0 Тогда
		...
		СвойстваКомандЭлементы = Неопределено;
		Если ПараметрыОбработки.СвойстваКомандЭлементы.Свойство(ИмяПараметра, СвойстваКомандЭлементы) Тогда
			СвойстваКомандЭлементы.Добавить(Новый Структура("Команда,Элемент", ИмяКоманды, Результат));
		Иначе
			СтруктураСвойства.Вставить(ИмяПараметра, Результат);
		КонецЕсли;
	Иначе
		...
		СвойстваКомандЭлементы = Неопределено;
		Если ПараметрыОбработки.СвойстваКомандЭлементы.Свойство(ИмяСвойства, СвойстваКомандЭлементы) Тогда
			СвойстваКомандЭлементы.Добавить(Новый Структура("Команда,Элемент", ИмяКоманды, ОписаниеЗначения));
		Иначе
			СтруктураСвойства.Вставить(ИмяСвойства, ОписаниеЗначения);
		КонецЕсли;
	КонецЕсли;
...
Процедура ИзменитьЭлементы(ПараметрыОбработки, МакетИзменений, ИмяРодителя = Неопределено, ШиринаИмени = Неопределено, НомСтроки = 2, НомКолонки = 1)
	...
	Если ТипЗнч(ПараметрыОбработки.ДанныеМакета) = Тип("Соответствие") Тогда
		Для Каждого КоллекцияИзменений Из МакетИзменений Цикл
			Если НЕ ПрименятьИзменение(Форма, "Элементы", КоллекцияИзменений) Тогда
				Продолжить;
			КонецЕсли;

			ИмяЭлемента = КоллекцияИзменений["Имя"];
			...
			Иначе
				...
				ПодчиненныеЭлементы = КоллекцияИзменений[""];
				Если ПодчиненныеЭлементы <> Неопределено Тогда
	...
	Иначе
		...
		Пока Истина Цикл
			КоллекцияИзменений = Новый Соответствие;
			КоллекцияИзменений.Вставить("Имя", ИмяЭлемента);
			Для Инд = ШиринаИмени + 1 По МакетИзменений.ШиринаТаблицы Цикл
				ИмяСвойства = МакетИзменений.Область(1, Инд).Текст;
				ОписаниеЗначения = МакетИзменений.Область(НомСтроки, Инд).Текст;
			...
			Иначе
				...
				Для Каждого ЭлементСоответствия Из КоллекцияИзменений Цикл
					...
					ИзменитьСвойствоЭлемента(ПараметрыОбработки, Элемент, ИмяЭлемента, Родитель
						,ЭлементСоответствия.Ключ, ЭлементСоответствия.Значение
						,СтруктураСвойстваРезультат, СтруктураДействия, СтруктураОперации);
				КонецЦикла;
...
Процедура ПрименитьИзмененияЭлемента(Форма, ПараметрыОбработки, Элемент, ИмяЭлемента, Родитель, ИмяРодителя, СтруктураСвойства, СтруктураДействия, СтруктураОперации, КэшРеквизиты)
	...
	Если Элемент = Неопределено Тогда
		Если СтруктураОперации.Свойство("Вместо") Тогда
			...
			Если ТипЗнч(ИсходныйЭлемент) = Тип("ПолеФормы")
				ИЛИ ТипЗнч(ИсходныйЭлемент) = Тип("ТаблицаФормы") Тогда
...
Функция ВычислитьЗначениеНастройки(ПараметрыОбработки, ИмяДействия, ТекстОписание, ТекущееЗначение = Неопределено)
	...
	ИначеЕсли ИмяДействия = "ОбработатьТекст" Тогда
		...
		Если ТипЗнч(ПараметрыОбработки.ДанныеМакета) = Тип("Соответствие") Тогда
			Для Каждого КоллекцияИзменений Из ПараметрыОбработки.ДанныеМакета[ИмяМакета] Цикл
				Если НЕ ПрименятьИзменение(ПараметрыОбработки.Форма, "ОбработкаТекста", КоллекцияИзменений) Тогда
					Продолжить;
				КонецЕсли;
				
				ОбработатьТекст(НовоеЗначение
					,ПолучитьЗначениеJSON(КоллекцияИзменений["Маркер"])
					,ПолучитьЗначениеJSON(КоллекцияИзменений["Текст"])
					,КоллекцияИзменений["Счетчик"]);
			КонецЦикла;
		Иначе
			...
			Для НомСтроки = 2 По МакетИзменений.ВысотаТаблицы Цикл
				...
				Условие = МакетИзменений.Область(НомСтроки, 4).Текст;
				
				Если ЗначениеЗаполнено(Условие) Тогда
					КоллекцияИзменений = Новый Соответствие;
					КоллекцияИзменений.Вставить("Маркер", Маркер);
					КоллекцияИзменений.Вставить("Текст", Текст);
					КоллекцияИзменений.Вставить("Условие()", Условие);
	...
	ИначеЕсли ИмяДействия = "Характеристика" Тогда
		...
		Иначе
			Возврат ПланыВидовХарактеристик[МассивПуть[0]][МассивПуть[1]].ТипЗначения;
		КонецЕсли;
...
Функция ПолучитьМакет(ИмяМакета) Экспорт
	МассивПуть = СтрРазделить(ИмяМакета, ".");
	Если МассивПуть.Количество() = 4  Тогда
		МенеджерОбъекта = МенеджерПоПолномуИмени(ИмяМакета);
		МакетИзменений = МенеджерОбъекта[МассивПуть[1]].ПолучитьМакет(МассивПуть[3]);
	Иначе
		МакетИзменений = ПолучитьОбщийМакет(МассивПуть[1]);
	КонецЕсли;
	
	Возврат МакетИзменений;
КонецФункции

Процедура ИзменитьФорму(Форма, Знач МакетИзменений = Неопределено, ИмяШаблона = Неопределено
	,Парам1 = Неопределено, Парам2 = Неопределено, Парам3 = Неопределено, Парам4 = Неопределено, Парам5 = Неопределено) Экспорт
	
	Если ТипЗнч(Парам1) = Тип("Соответствие") Тогда
		СоответствиеПараметры = Парам1;
	Иначе
		СоответствиеПараметры = Новый Соответствие;
	...
	Если ТипЗнч(МакетИзменений) = Тип("Строка") Тогда
		ИмяМакета = МакетИзменений;
		МакетИзменений = ПолучитьМакет(МакетИзменений);
	КонецЕсли;
	...
	СвойстваКомандЭлементы = Новый Структура;
	СвойстваКомандЭлементы.Вставить("ИспользуемаяТаблица", Новый Массив);
	
	ПараметрыОбработки = Новый Структура("Форма,ДанныеМакета,ДобавленныеЭлементы,ЗависимыеСвойстваЭлементов,СвойстваКомандЭлементы"
		,Форма
		,ДанныеМакета
		,Новый Соответствие
		,ЗависимыеСвойстваЭлементов
		,СвойстваКомандЭлементы);
	...
	Для Каждого КлючИЗначение Из ПараметрыОбработки.СвойстваКомандЭлементы Цикл
		Для Каждого ИмяЗначение Из КлючИЗначение.Значение Цикл
			Форма.Команды[ИмяЗначение.Команда][КлючИЗначение.Ключ] = Форма.Элементы[ИмяЗначение.Элемент];
		КонецЦикла;
	КонецЦикла;

	Если ИмяМакета <> Неопределено Тогда
		ПослеПримененияМакета(Форма, ИмяМакета, ИмяШаблона, СоответствиеПараметры);
	КонецЕсли;
КонецПроцедуры

Процедура ПослеПримененияМакета(Форма, ИмяМакета, ИмяШаблона, СоответствиеПараметры)
КонецПроцедуры
...
Функция ПрименятьИзменение(Форма, ТипОбласти, Изменения)
	Условие = Изменения["Условие()"];
	Если Условие <> Неопределено Тогда
		Изменения.Удалить("Условие()");
		Возврат Вычислить(Условие);
	Иначе
		Возврат Истина;
	КонецЕсли;
КонецФункции
